var jsPsychSVOSurveyLikert = (function (jspsych) {
  'use strict';
  ...
  class SVOSurveyLikertPlugin {
      ...
      trial(display_element, trial) {
          ...
          html +=
              ".jspsych-survey-likert-statement { display:inline-block; font-size: 16px; padding-top: 40px; margin-bottom:10px; }" +
                  ".jspsych-survey-likert-opts { list-style:none; width:" +
                  w +
                  "; margin:auto; padding:0 0 35px; display:block; font-size: 16px; line-height:1.1em; }" +
                  ".jspsych-survey-likert-opt-label { line-height: 1.6em; color: #444; }" +
                  ".jspsych-survey-likert-opts:before { content: ''; position:relative; top:11px; /*left:9.5%;*/ display:block; background-color:#efefef; height:4px; width:100%; }" +
                  ".jspsych-survey-likert-opts:last-of-type { border-bottom: 0; }" +
                  ".jspsych-survey-likert-opts li { display:inline-block; /*width:19%;*/ text-align:center; vertical-align: top; }" +
                  ".jspsych-survey-likert-opts li input[type=radio] { display:block; position:relative; top:0; left:50%; margin-left:-6px; margin-bottom:-20px; }";
          html += "</style>";
          // show preamble text
          ...
          if (trial.autocomplete) {
              html += '<form id="jspsych-survey-likert-form" style="min-width:800px;">';
          }
          else {
              html += '<form id="jspsych-survey-likert-form"  style="min-width:800px;" autocomplete="off">';
          }
          ...
          for (var i = 0; i < trial.questions.length; i++) {
              ...
              // add question
              html += '<div> <label class="jspsych-survey-likert-statement" style="display: inline-block; white-space: break-spaces; vertical-align: middle; margin-top: 0px;  padding-top: 91px;  ">' + question.prompt + "</label>";
              // add options
              var width = 100 / question.labels.length;
              var options_string = '<ul class="jspsych-survey-likert-opts" style="width: 80%; display: inline-block;" data-name="' +
                  question.name +
                  '" data-radio-group="Q' +
                  question_order[i] +
                  '">';
              for (var j = 0; j < question.labels.length; j++) {
                  options_string +=
                      '<li style="width:' +
                          width +
                          '%"><label class="jspsych-survey-likert-opt-label" style="white-space: break-spaces;"><input type="radio" name="Q' +
                          question_order[i] +
                          '" value="' +
                          j +
                          '"';
  ...
  SVOSurveyLikertPlugin.info = info;

  return SVOSurveyLikertPlugin;
